...
// =============================================================
// QUERY: Get product by ID (cached with custom key)
// =============================================================

[Cacheable(120, KeyPrefix = "product")]
public record GetProductByIdQuery(Guid ProductId) : IQuery<ProductDto?>, ICacheKeyProvider
{
...
public class GetProductByIdHandler : IQueryHandler<GetProductByIdQuery, ProductDto?>
{
    ...
    {
        Console.WriteLine($"  [Handler] GetProductById executing (cache miss) - {query.ProductId}");
...
// =============================================================
// QUERY: Search products (cached with sliding expiration)
// =============================================================

[Cacheable(60, UseSlidingExpiration = true)]
...
public class SearchProductsHandler : IQueryHandler<SearchProductsQuery, ProductSearchResult>
{
    ...
    {
        Console.WriteLine($"  [Handler] SearchProducts executing (cache miss) - term={query.SearchTerm}");
...
// =============================================================
// QUERY: Get categories (cached for 5 minutes)
// =============================================================

[Cacheable(300, KeyPrefix = "categories")]
public record GetCategoriesQuery() : IQuery<IReadOnlyList<string>>;

public class GetCategoriesHandler : IQueryHandler<GetCategoriesQuery, IReadOnlyList<string>>
{
    public ValueTask<IReadOnlyList<string>> Handle(GetCategoriesQuery query, CancellationToken ct)
    {
        Console.WriteLine("  [Handler] GetCategories executing (cache miss)");
        var categories = ProductStore.Products.Values
            .Select(p => p.Category)
            .Distinct()
            .OrderBy(c => c)
            .ToList();

        return new ValueTask<IReadOnlyList<string>>(categories);
    }
}

// =============================================================
// COMMAND: Create product (not cached)
// =============================================================
